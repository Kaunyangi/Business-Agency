...
router.get('/', (req, res) => {
  ...
  const trips = db.prepare(`SELECT * FROM bus_trips ${where} ORDER BY trip_date, departs_at`).all(...params);
  res.json({ trips });
});

router.get('/:id/seats', (req, res) => {
  const trip = db.prepare('SELECT * FROM bus_trips WHERE id = ?').get(req.params.id);
  if (!trip) return res.status(404).json({ error: 'Trip not found' });
  const seats = db.prepare('SELECT id, row_no, letter, status FROM bus_seats WHERE trip_id = ? ORDER BY row_no, letter').all(trip.id);
  res.json({ trip, seats });
});
...
router.post('/checkout', requireAuth, idempotent, validate(bookSchema), async (req, res, next) => {
  try {
    ...
    const built = transaction(() => {
      const trip = db.prepare('SELECT * FROM bus_trips WHERE id = ?').get(tripId);
      if (!trip) throw Object.assign(new Error('Trip not found'), { status: 404 });

      let subtotalCents = 0;
      const lineItems = [];
      for (const seatId of seatIds) {
        const seat = db.prepare('SELECT * FROM bus_seats WHERE id = ? AND trip_id = ?').get(seatId, tripId);
        ...
        db.prepare('UPDATE bus_seats SET status = ? WHERE id = ?').run('booked', seat.id);
        subtotalCents += trip.fare_cents;
        lineItems.push({ ref_type: 'bus_seat', ref_id: seat.id, description: `${trip.operator} ${trip.coach_no} · ${trip.trip_date} ${trip.departs_at} — Seat ${seat.row_no}${seat.letter}`, unit_price_cents: trip.fare_cents, quantity: 1, line_total_cents: trip.fare_cents });
      }
      const { amountCents: commissionCents, ruleKey } = revenue.computeCommission('bus', subtotalCents);
      ...
      db.prepare(
        `INSERT INTO orders (id, user_id, order_type, status, subtotal_cents, commission_cents, fees_cents, total_cents, idempotency_key, buyer_name, buyer_phone, buyer_email, buyer_id_number, created_at)
         VALUES (?,?, 'bus', 'pending', ?, ?, 0, ?, ?, ?, ?, ?, ?, datetime('now'))`
    ...
    const paymentResult = await charge({ method: paymentMethod, amountCents: built.totalCents, payerRef, description: `Motion bus — ${built.trip.operator} ${built.trip.coach_no}` });

    if (paymentResult.status !== 'success') {
      transaction(() => {
        built.seatIds.forEach((sid) => db.prepare('UPDATE bus_seats SET status = ? WHERE id = ?').run('available', sid));
    ...
    audit(req, req.user.id, 'bus.book', { orderId: built.orderId, totalCents: built.totalCents });
